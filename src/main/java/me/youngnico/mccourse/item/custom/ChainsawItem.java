...
public class ChainsawItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        // This thing happens only sever side
        // because world.isClient() return true if the action happens client side
        if (!world.isClient()) {
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        if (!Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("tooltip.mccourse.shift"));
        } else {
            tooltip.add(Text.translatable("tooltip.mccourse.chainsaw.tip1"));
            tooltip.add(Text.translatable("tooltip.mccourse.chainsaw.tip2"));
        }
